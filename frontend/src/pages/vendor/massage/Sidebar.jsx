...
const Sidebar = ({ 
  ...
  isOpen, 
  onClose, 
  shopName = "AIU Wellness & Cupping Therapy",
  isCollapsed = false,
  onToggleCollapse
}) => {
      ...
      `}>
        <div>
          ...
          <div className={`overflow-y-auto ${isCollapsed ? 'py-3 px-2' : 'py-5 px-3'} max-h-[calc(100vh-140px)] space-y-1`}>
            ...
            <SidebarSection title="Therapy & Bookings" isCollapsed={isCollapsed}>
              <SidebarItem icon={CalendarCheck} label="Therapy Appointments" id="appointments" activeTab={activeTab} onTabChange={handleTabChange} isCollapsed={isCollapsed} />
          ...
          {!isCollapsed ? (
            ...
            <>
              <div 
                ...
                title={`${user?.firstName || user?.name || 'Luqman Hakim'} (${user?.email || ''})`}
              >
                {user?.firstName?.[0] || user?.name?.[0] || 'M'}
              </div>
              <button 
