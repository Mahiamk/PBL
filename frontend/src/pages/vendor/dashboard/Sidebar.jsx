      ...
      `}>
        ...
        <div>
          ...
          <div className={`overflow-y-auto ${isCollapsed ? 'py-3 px-2' : 'py-5 px-3'} max-h-[calc(100vh-140px)] space-y-1`}>
            <SidebarSection title="Performance" isCollapsed={isCollapsed}>
              ...
              <SidebarItem icon={PlusCircle} label="Add New Product" id="new-product" activeTab={activeTab} onTabChange={handleTabChange} isCollapsed={isCollapsed} />
            </SidebarSection>
            
            <SidebarSection title="Inventory & Catalog" isCollapsed={isCollapsed}>
              <SidebarItem icon={Package} label="Product Catalog" id="products" activeTab={activeTab} onTabChange={handleTabChange} isCollapsed={isCollapsed} />
              ...
              <SidebarItem icon={SlidersHorizontal} label="Attributes & Options" id="attributes" activeTab={activeTab} onTabChange={handleTabChange} isCollapsed={isCollapsed} />
              <SidebarItem icon={Star} label="Featured Items" id="featured" activeTab={activeTab} onTabChange={handleTabChange} isCollapsed={isCollapsed} />
            </SidebarSection>

            <SidebarSection title="Sales & Clients" isCollapsed={isCollapsed}>
              <SidebarItem icon={Receipt} label="Orders & Billing" id="orders" activeTab={activeTab} onTabChange={handleTabChange} isCollapsed={isCollapsed} />
          ...
          {!isCollapsed ? (
              ...
              <div className="flex items-center justify-between px-2 py-1 mb-2">
                <div className="flex items-center space-x-2 truncate">
                  <div className="w-7 h-7 rounded-full bg-[#f5edf0] text-[#594951] font-bold text-xs flex items-center justify-center shrink-0">
                    {user?.firstName?.[0] || user?.name?.[0] || 'V'}
                  </div>
                  <div className="truncate">
            ...
            <>
              <div 
                ...
                title={`${user?.firstName || user?.name || 'Vendor'} (${user?.email || ''})`}
              >
                {user?.firstName?.[0] || user?.name?.[0] || 'V'}
              </div>
              <button 
